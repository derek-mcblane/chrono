...
void ChMeshFileLoader::FromAbaqusFileNodeSetsElementSets(std::shared_ptr<ChMesh> mesh,
                                      const char* filename,
                                      std::shared_ptr<ChContinuumMaterial> my_material,
                                      std::map<std::string, std::vector<std::shared_ptr<ChNodeFEAbase>>>& node_sets,
        ...
        if (line[0] == '*') {
            ...
            if (line.find("*ELEMENT") == 0) {
                ...
                if (nse > 0) {
                    ...
                    std::cout << "| parsing element set: " << s_ele_set << std::endl;
                    auto new_elset =
                        element_sets.insert({s_ele_set, std::vector<std::shared_ptr<ChElementBase>>()});
                    if (new_elset.second) {
                        current_elementset_vector = &new_elset.first->second;
                    } else
                        throw std::invalid_argument(
                            "ERROR in .inp file: multiple NSET with same name has been specified\n");
                }
